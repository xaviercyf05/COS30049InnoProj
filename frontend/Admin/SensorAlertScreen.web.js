  ...
  <body>
    ...
    <script>
      ...
      alerts.forEach((alert) => {
        ...
        const key = String(alert.parkName || alert.location || (String(alert.latitude) + ',' + String(alert.longitude))).trim().toLowerCase();
        if (!parkMarkers.has(key)) {
          parkMarkers.set(key, { alert, count: 0, latitude: alert.latitude, longitude: alert.longitude });
        }
