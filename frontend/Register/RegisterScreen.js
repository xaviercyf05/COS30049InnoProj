...
export default function RegisterScreen({ navigation }) {
  ...
  const [isOldGuide, setIsOldGuide] = useState(null);
  ...
  const toggleParkSelection = (park) => {
    setSelectedParks((prev) =>
      prev.includes(park) ? prev.filter((p) => p !== park) : [...prev, park],
    );
  };

  const pickDocument = async (isCert) => {
    try {
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        if (isCert) setCertFile(fileData);
        else setResumeFile(fileData);
        setErrorMessage("");
      }
    } catch (error) {
  ...
  const getValidationError = () => {
    ...
    if (isOldGuide === null)
      return "Please select if you are a New or Old Park Guide.";
    if (isOldGuide) {
      if (selectedParks.length === 0)
        return "Please select which park(s) you currently guide.";
      if (!certFile)
        return "Please upload your Park Guide Certification as proof.";
    } else {
  ...
  const handleSubmit = async () => {
    ...
    const createFormPayload = () => {
      ...
      payload.append("isOldGuide", isOldGuide.toString());

      if (isOldGuide) {
        payload.append("assignedParks", JSON.stringify(selectedParks));
        const file = certFile;
        payload.append(
          "certification",
          file.file || { uri: file.uri, name: file.name, type: file.mimeType },
        );
      } else {
    ...
    try {
      for (const baseUrl of baseUrls) {
        try {
          const candidateResponse = await fetch(
            `${baseUrl}/api/v1/public/register`,
            {
              method: "POST",
      ...
      if (!response) {
        throw (
          lastNetworkError || new Error("Unable to reach registration service.")
        );
      }
      ...
      >
        ...
        <View style={styles.requirementsCard}>
          ...
          <Text style={styles.noteText}>
            Your application will be reviewed by the Sarawak Forestry
            Corporation admin team.
          </Text>
        </View>
        ...
        <View style={styles.formCard}>
          ...
          <Text style={styles.label}>
            Are you an Existing or New Park Guide?
          </Text>
          <View style={styles.toggleRow}>
            <TouchableOpacity
              style={[
                styles.toggleBtn,
                isOldGuide === false && styles.toggleBtnActive,
              ]}
              onPress={() => setIsOldGuide(false)}
            >
              <Text
                style={[
                  styles.toggleBtnText,
                  isOldGuide === false && styles.toggleBtnTextActive,
                ]}
              >
                New Guide
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.toggleBtn,
                isOldGuide === true && styles.toggleBtnActive,
              ]}
              onPress={() => setIsOldGuide(true)}
            >
              <Text
                style={[
                  styles.toggleBtnText,
                  isOldGuide === true && styles.toggleBtnTextActive,
                ]}
              >
                Old Guide
              </Text>
          ...
          {isOldGuide === true && (
            <View style={{ marginTop: 12 }}>
              <Text style={styles.label}>Select Your Current Park(s)</Text>
                ...
                {PARKS.map((park) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.parkChip,
                        isSelected && styles.parkChipActive,
                      ]}
                      onPress={() => toggleParkSelection(park)}
                    >
                      <Text
                        style={[
                          styles.parkChipText,
                          isSelected && styles.parkChipTextActive,
                        ]}
                      >
                        {park}
                      </Text>
          ...
          {isOldGuide !== null && (
            <View style={{ marginTop: 12 }}>
              <Text style={styles.label}>
                {isOldGuide ? "Park Guide Certificate (PDF)" : "Resume (PDF)"}
              </Text>
              <TouchableOpacity
                style={styles.uploadButton}
                onPress={() => pickDocument(isOldGuide)}
              >
...
const styles = StyleSheet.create({
  ...
  toggleBtnText: { fontWeight: "700", color: COLORS.muted },
  ...
  parkChipText: { fontSize: 12, color: COLORS.body, fontWeight: "600" },
  parkChipTextActive: { color: "#FFFFFF" },
